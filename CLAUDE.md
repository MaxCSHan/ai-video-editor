...
6. **Phase 2** (`editorial_agent.py` → `editorial_prompts.py`) — Multi-call split pipeline by default for Gemini: Call 2A (freeform reasoning, high temp) → Call 2A.5 (structuring into StoryPlan JSON) → Call 2B (precise timestamp assembly). Single-call fallback via `--single` flag. All clip reviews + transcripts + briefing → `EditorialStoryboard` Pydantic model. Visual mode (≤10 clips) attaches proxy videos.
