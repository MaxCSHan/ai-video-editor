"""Gemma pattern — frame-based analysis using a local model via OpenAI-compatible API."""
...
def get_client(cfg: GemmaConfig):
...
def _build_image_content(image_path: Path, timestamp_fmt: str) -> list[dict]:
    """Build an OpenAI vision-format content block for one image with its timestamp label."""
    return [
    ...
    content.append(
        {
            "type": "text",
            "text": (
                f"These are {len(batch_frames)} frames from a "
                f"{format_duration(video_info['duration_sec'])} video, "
                f"covering {time_start} to {time_end} "
                ...
                "Group consecutive similar frames into single shots. "
                "Be specific about what changes between shots."
            ),
        }
    )
...
def run_gemma_analysis(
    ...
    """Full Gemma pipeline: batch analyze frames -> synthesize -> write storyboard."""
    ...
    print(f"  Analyzing {len(all_frames)} frames in {len(batches)} batch(es) with Gemma...")
    batch_analyses = []
    ...
    output_path = storyboard_dir / "storyboard_gemma.md"
